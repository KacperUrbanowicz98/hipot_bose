...
class MainScreen(ctk.CTkFrame):
    def __init__(self, parent, hrid: str, user: dict, on_logout):
        ...
        self._running  = False
        self._abort    = threading.Event()   # ← flaga przerwania
    ...
    def _build(self):
        ...
        # ── Szczegóły Ground Bond (ukryte gdy brak GND w profilu) ──────────
        ...
        self.gnd_frame.grid_remove()   # domyślnie ukryty
        ...
        btn_row.grid(row=6, column=0, sticky="ew", pady=(0, 0))
        btn_row.grid_columnconfigure(0, weight=1)

        self.test_btn = ctk.CTkButton(
            btn_row, text="▶ START TEST",
    ...
    def _start_test(self):
        ...
        self._running = True
        self._abort.clear()   # ← wyczyść flagę przed startem

        self.test_btn.configure(state="disabled", text="⏳ Test w toku...")
        self.abort_btn.configure(state="normal")   # ← aktywuj ABORT
    ...
    def _abort_test(self):
        """Ustawia flagę przerwania — wątek testowy sprawdza ją między krokami."""
        self._abort.set()
        self.abort_btn.configure(state="disabled", text="⏹ Przerywanie...")
        self._set_status("⚠ Przerwanie testu — czekam na zakończenie kroku...",
                         COLORS["warning"])
    ...
    def _run_thread(self, sn: str):
        ...
        ctrl = HipotController(
            ...
            relay_port=serial_cfg.get("relay_port", None),   # ← przekaż relay_port
        )
        ...
        try:
            ...
            # ── Sprawdź abort przed startem ────────────────────────────────
            if self._abort.is_set():
                self.after(0, self._set_status,
                           "⚠ Test anulowany przed startem.", COLORS["warning"])
                return
            ...
            # ── Sprawdź abort po HiPot (przed GND) ────────────────────────
            if self._abort.is_set():
                self.after(0, self._set_status,
                           "⚠ Test przerwany przez operatora.", COLORS["warning"])
                self.after(0, self.result_label.configure,
                           {"text": "ABORT", "text_color": COLORS["warning"]})
                return

            self.after(0, self._show_result, sn, hipot_result, gnd_result)
    ...
    def _show_result(self, sn: str, r: dict, gnd: dict = None):
        error  = r.get("error")
        result = r.get("result", "")
        ...
        if error:
            ...
            self._set_status(
                f"✘ FAIL | SN: {sn}{' | ' + desc if desc else ''}",
                COLORS["fail"]
            )
        else:
            status = r.get("status", "").upper()
            self.result_label.configure(
                text=f"⚠ {status}", text_color=COLORS["warning"]
            )
        ...
        if gnd is not None:
            ...
            if gnd_error:
                ...
                self._set_status(f"❌ GND Error: {gnd_error}", COLORS["fail"])
            elif gnd_verdict == "Pass":
                self.gnd_result_lbl.configure(
                    text="✔ GND PASS", text_color=COLORS["success"]
                )
            elif gnd_verdict == "Fail":
                self.gnd_result_lbl.configure(
                    text="✘ GND FAIL", text_color=COLORS["fail"]
                )
                desc = gnd.get("error_desc", "")
                self._set_status(
                    f"✘ GND FAIL | SN: {sn}{' | ' + desc if desc else ''}",
                    COLORS["fail"]
                )
            else:
                self.gnd_result_lbl.configure(
                    text="⚠ GND ?", text_color=COLORS["warning"]
                )

            self.gnd_res_lbl.configure(
                text=f"{gnd.get('resistance', '—')} mΩ"
            )
            self.gnd_curr_lbl.configure(
                text=f"{gnd.get('current', '—')} A"
            )
            self.gnd_time_lbl.configure(
                text=f"{gnd.get('time', '—')} s"
            )