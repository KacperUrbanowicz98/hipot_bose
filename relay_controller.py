...
Komendy ESP:
  PE          → przekaźnik na PE (Ground Bond)    → odpowiedź: OK:PE
  HIPOT       → przekaźnik na HiPot               → odpowiedź: OK:HIPOT
  STATUS?     → zapytanie o stan                  → odpowiedź: STATE:PE / STATE:HIPOT
  PING        → heartbeat / reset watchdog        → odpowiedź: PONG
  WATCHDOG:OFF → wyłącz watchdog                  → odpowiedź: OK:WATCHDOG_OFF
"""
...
class RelayController:
    ...
    # ── Połączenie ─────────────────────────────────────────────────────────
    def connect(self):
        """Otwiera port COM i czeka na READY od ESP."""
        ...
        try:
            ...
            # ESP po otwarciu portu wysyła READY + STATE:HIPOT
            # Czekamy max 2s na READY, nie blokujemy jeśli nie przyjdzie
            self._wait_for_ready(timeout=2.0)
        except serial.SerialException as e:
            raise RelayError(f"Błąd otwarcia portu ESP {self.port}: {e}")

    def _wait_for_ready(self, timeout: float = 2.0):
        """Czeka na komunikat READY z ESP po otwarciu portu."""
        ...
        while time.time() < deadline:
            ...
            if "READY" in decoded:
                return
        # Brak READY — stary firmware lub bufor już odczytany, kontynuujemy
    ...
    def set_pe(self) -> bool:
        ...
        ESP weryfikuje sprzętowo i odpowiada OK:PE lub ERR:RELAY_FAIL:PE.
        Zwraca True jeśli OK, rzuca RelayError jeśli FAIL.
        """
    ...
    def set_hipot(self) -> bool:
        """
        Przełącza przekaźnik na HIPOT (pozycja domyślna).
        Zwraca True jeśli OK, rzuca RelayError jeśli FAIL.
        """
    ...
    def get_status(self) -> str:
        """Zwraca aktualny stan: 'PE', 'HIPOT' lub 'UNKNOWN'."""
        try:
    ...
    def watchdog_off(self):
        """Wyłącza watchdog ESP (np. podczas długich testów)."""
